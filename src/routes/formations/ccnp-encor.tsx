...
export const Route = createFileRoute('/formations/ccnp-encor')({
  component: FormationPage,
  head: () => ({
    meta: [
      { title: "CCNP Enterprise Core — ENCOR 350-401 v9 | GALACTUS Digital" },
      { name: "description", content: "CCNP Enterprise : Core Networking (ENCOR 350-401) v9. 70h. SD-Access, LISP/VXLAN, assurance réseau, automatisation. Formateur CCNP/CCDP. Qualiopi." },
    ],
  }),
})
...
const DATA = {
  editeur: 'Cisco',
  famille: 'Cisco Networking Academy — CCNP Enterprise',
  titre: 'CCNP Enterprise — Core Networking (ENCOR)',
  sousTitre: 'ENCOR 350-401 v9 · Cours 1 sur 2 · Formateur CCNP/CCDP certifié',
  ref: 'CCNP-ENCOR-001',
  niveau: 'Avancé',
  ...
  description: `Premier cours de la série CCNP Enterprise (version 9), ENCOR constitue l'examen cœur. Il couvre l'architecture réseau d'entreprise, la double pile IPv4/IPv6, la virtualisation, l'infrastructure (commutation, routage, sans-fil), l'assurance réseau, la sécurité et l'automatisation. Depuis la version v1.2 (19 mars 2026), le contenu sans-fil est retiré d'ENCOR et migré vers la piste dédiée CCNP Wireless (examen 350-101 WLCOR) ; l'examen est recentré sur le cœur d'entreprise, avec un poids accru sur le SD-WAN, l'automatisation, l'architecture Zero Trust et les opérations pilotées par l'IA. Il prépare à l'examen Cisco 350-401 (blueprint v1.2) et constitue le socle du CCNP Enterprise ainsi que du CCIE Enterprise. Formation dispensée par un formateur Cisco CCNP/CCDP certifié. Prérequis recommandé : CCNA validé.`,
  objectifs: [
    "Concevoir l'architecture réseau d'entreprise — hiérarchie, campus, SD-WAN",
    { text: "Maîtriser SD-Access : rôles Fabric Edge / Control Plane / Border, LISP (identité) et VXLAN (localisation)", neuf: true },
    "Configurer le routage avancé — OSPF, EIGRP, BGP et redistribution",
    "Déployer la virtualisation — VRF, GRE, IPsec, tunnels",
    { text: "Mettre en œuvre l'assurance réseau — télémétrie, monitoring, diagnostic", neuf: true },
    { text: "Concevoir une architecture de sécurité Zero Trust", neuf: true },
    { text: "Comprendre les opérations réseau pilotées par l'IA (AI-driven network operations)", neuf: true },
    "Mettre en œuvre la sécurité d'infrastructure et l'automatisation (Python, REST API, JSON)",
  ] as Item[],
  public: [
    "Administrateurs et ingénieurs réseau certifiés CCNA",
    "Techniciens réseau expérimentés visant le niveau professionnel",
    "Ingénieurs infrastructure en environnement d'entreprise",
    "Candidats à la certification CCNP Enterprise",
  ],
  prerequis: [
    "CCNA (200-301) validé ou compétences équivalentes",
    "Expérience pratique du routage et de la commutation Cisco",
    "Maîtrise de la configuration IOS/IOS-XE en CLI",
    "3 à 5 ans d'expérience réseau recommandés",
  ],
  modules: [
    { num: '01', titre: 'Architecture & conception', duree: '16h', contenu: [
      "Modèles hiérarchiques — 3-tier, collapsed core, campus",
      { text: "SD-Access Fabric — Fabric Edge, Control Plane node, Border node", neuf: true },
      { text: "LISP — enregistrement des mappages identité (EID) vers localisation (RLOC)", neuf: true },
      { text: "VXLAN — encapsulation et transport dans le fabric", neuf: true },
    ] as Item[] },
    { num: '02', titre: 'Virtualisation & infrastructure', duree: '18h', contenu: [
      "Virtualisation — VRF, GRE, IPsec, tunnels",
      "Commutation avancée — STP, EtherChannel, first-hop redundancy (HSRP/VRRP)",
      "Routage — OSPF, EIGRP, BGP et redistribution",
      { text: "Sans-fil retiré d'ENCOR v1.2 → piste dédiée CCNP Wireless (350-101 WLCOR)", neuf: true },
    ] as Item[] },
    { num: '03', titre: 'Assurance réseau & sécurité', duree: '18h', contenu: [
      { text: "Assurance réseau — télémétrie, monitoring, IP SLA, NetFlow", neuf: true },
      "Sécurité d'infrastructure — AAA, control plane policing, ACL",
      "Sécurité des équipements — durcissement, contrôle d'accès",
      { text: "Architecture de sécurité Zero Trust", neuf: true },
      "Dual-stack IPv4/IPv6 — bonnes pratiques",
    ] as Item[] },
    { num: '04', titre: 'Automatisation & programmabilité', duree: '18h', contenu: [
      "Automatisation — Python, REST API, JSON, EEM",
      { text: "Modèles de données et programmabilité (YANG, NETCONF/RESTCONF)", neuf: true },
      "Outils de gestion et contrôleurs (Catalyst Center)",
      { text: "Opérations réseau pilotées par l'IA (AI-driven network operations)", neuf: true },
      "TP de synthèse — scénarios d'entreprise",
    ] as Item[] },
  ],
  examen: {
    code: '350-401 ENCOR v1.2',
    duree: '120 minutes',
    langue: 'Anglais',
    organisme: 'Cisco (Pearson VUE)',
    format: 'QCM, glisser-déposer et simulations',
    score: 'Défini par Cisco (variable)',
    note: `ENCOR 350-401 est l'examen cœur du CCNP Enterprise. Sa réussite octroie la certification Cisco Certified Specialist – Enterprise Core et satisfait l'exigence d'examen cœur du CCNP Enterprise (à compléter par une concentration, ex. ENARSI). Il compte aussi vers le CCIE Enterprise. Le passage de l'examen (voucher officiel) est inclus dans le tarif de la formation.`,
  },
  methodes: [
    ...
    "Supports officiels Cisco Press et blueprints d'examen à jour (v1.2)",
    ...
    "Préparation intensive à l'examen officiel Cisco",
...
function FormationPage() {
  const FORMATION = getFormationBySlug('ccnp-encor')
          ...
          <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
            ...
            <div style={{ display: 'flex', gap: '2rem', flexWrap: 'wrap' }}>
              {[
                ...
                { icon: <Calendar size={14} />, text: 'Voir calendrier', href: '/calendrier?f=ccnp-encor' },
              ].map((m, i) => (
                m.href ? (
        ...
        <section style={{ background: 'var(--g-white)', padding: '1.5rem 2rem', borderBottom: '1px solid rgba(187,187,187,0.25)' }}>
          <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap' }}>
              <span style={{ fontFamily: 'var(--font-title)', fontSize: '0.68rem', fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: '#888' }}>Modalités & tarifs :</span>
              {TARIFS.map(t => (
                <div key={t.modalite} style={{ display: 'inline-flex', alignItems: 'center', gap: '0.55rem', padding: '0.4rem 0.9rem', border: '1px solid rgba(187,187,187,0.5)', borderLeft: '3px solid var(--g-red)', background: 'var(--g-offwhite)' }}>
                  <span style={{ fontSize: '0.78rem', fontFamily: 'var(--font-title)', fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase', color: '#5a5a58' }}>{t.label}</span>
                  <span style={{ fontFamily: 'var(--font-title)', fontSize: '0.80rem', fontWeight: 700, color: t.surDevis ? '#888' : 'var(--g-red)', fontStyle: t.surDevis ? 'italic' : 'normal' }}>{t.surDevis ? 'Sur devis' : t.valeur}</span>
                </div>
              ))}
              <a href="mailto:president@galactusdigital.com" style={{ fontSize: '0.74rem', fontFamily: 'var(--font-title)', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--g-red)', textDecoration: 'none', marginLeft: '0.25rem' }}>Demander un devis →</a>
            </div>
          </div>
        </section>
