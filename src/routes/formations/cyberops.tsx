...
export const Route = createFileRoute('/formations/cyberops')({
...
const DATA = {
  ...
  objectifs: [
    ...
    'Mettre en œuvre la surveillance réseau avec des systèmes IDS/IPS et des flux NetFlow',
    'Effectuer l\'analyse forensique d\'hôtes compromis — analyse de malwares, logs Windows/Linux',
    'Appliquer les frameworks de réponse aux incidents (NIST SP 800-61) dans un contexte SOC',
    'Comprendre et appliquer les politiques de sécurité, conformité et procédures d\'escalade SOC',
  ],
  public: ['Analystes SOC niveau 1 et 2', 'Techniciens sécurité réseau', 'Administrateurs systèmes souhaitant évoluer en cybersécurité', 'Professionnels IT en reconversion cybersécurité'],
  prerequis: ['Connaissance des réseaux TCP/IP (niveau CCNA ITN recommandé)', 'Notions de systèmes d\'exploitation Windows et Linux', 'Compréhension basique des protocoles réseau (HTTP, DNS, SMTP)'],
  modules: [
    { num: '01–03', titre: 'Concepts de sécurité', duree: '12h', contenu: ['CIA Triad — confidentialité, intégrité, disponibilité', 'Gestion des risques — évaluation, mitigation, frameworks ISO 27001', 'Cryptographie — chiffrement symétrique/asymétrique, PKI, hachage', 'TP — Analyse de certificats et outils cryptographiques'] },
    { num: '04–07', titre: 'Surveillance de la sécurité réseau', duree: '16h', contenu: ['Protocoles réseau et analyse de trafic avec Wireshark et tcpdump', 'Technologies SOC — SIEM, IDS/IPS, honeypots', 'Analyse de fichiers PCAP et détection d\'anomalies', 'Surveillance avec NetFlow et flux de données réseau'] },
    { num: '08–10', titre: 'Analyse basée sur l\'hôte', duree: '12h', contenu: ['Analyse forensique Windows — registre, logs d\'événements, artefacts', 'Analyse forensique Linux — logs système, commandes d\'investigation', 'Analyse de malwares — types, comportements, outils de détection', 'TP — Analyse d\'incident sur hôte compromis'] },
  ...
  examen: { code: '200-201 CBROPS', duree: '120 minutes', langue: 'Français', organisme: 'Cisco (Pearson VUE)', format: 'QCM, simulation d\'analyse réseau', score: '750 / 1000', note: 'L\'examen 200-201 CBROPS est indépendant. Un seul examen suffit pour obtenir la certification CCNA Cybersecurity, sans module préalable obligatoire. Le passage de l\'examen (voucher officiel) est inclus dans le tarif de la formation.' },
  methodes: [
...
function CyberOpsPage() {
  const FORMATION = getFormationBySlug('cyberops')
  ...
  return (
    <>
      ...
      <main style={{ paddingTop: '72px' }}>
          ...
          <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
            ...
            <div style={{ background: 'var(--g-white)', border: '1px solid rgba(187,187,187,0.3)', borderLeft: '4px solid var(--g-red)', padding: '2rem', display: 'flex', gap: '2rem' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ marginTop: '1rem', padding: '0.75rem 1rem', background: 'rgba(228,31,38,0.06)', border: '1px solid rgba(228,31,38,0.15)' }}><p style={{ fontSize: '0.84rem', color: '#5a5a58', margin: 0 }}><strong style={{ color: 'var(--g-black)' }}>Note — </strong>{DATA.examen.note}</p></div>
              </div>
            </div>
          </div>
        </section>
        ...
        <section style={{ background: 'var(--g-offwhite)', padding: '4rem 2rem' }}>
          <div style={{ maxWidth: '1280px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '3rem' }} className="fiche-public">
            <div><SectionTitle>Conformité réglementaire</SectionTitle><div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>{[{ ref: 'Art. L.6313-1', title: 'Action de formation', desc: 'Formation reconnue au sens du Code du travail.' }, { ref: 'Qualiopi Ind. 4', title: 'Convocation', desc: 'Convocation complète envoyée avant démarrage.' }, { ref: 'Critère 3 Qualiopi', title: 'Évaluation des acquis', desc: 'Quiz et auto-évaluation selon 4 niveaux.' },                   { ref: 'Circ. DGEFP/MOC/2026/30', title: 'Plan de travail individuel', desc: "Un plan de travail individuel est remis à chaque apprenant avant le démarrage de la formation FOAD ou e-learning, conformément à la circulaire DGEFP du 17 février 2026." },
                  { ref: 'Ind. 19 Qualiopi', title: 'Plateau technique', desc: 'Accès NetAcad et outils vérifiés avant démarrage.' }].map(c => (<div key={c.ref} style={{ background: 'var(--g-white)', border: '1px solid rgba(187,187,187,0.3)', padding: '1rem', display: 'flex', gap: '0.75rem' }}><div style={{ flexShrink: 0 }}><div style={{ fontFamily: 'var(--font-title)', fontSize: '0.62rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--g-red)', background: 'rgba(228,31,38,0.08)', padding: '0.2rem 0.55rem', whiteSpace: 'nowrap' }}>{c.ref}</div></div><div><div style={{ fontFamily: 'var(--font-title)', fontSize: '0.82rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--g-black)', marginBottom: '0.2rem' }}>{c.title}</div><p style={{ fontSize: '0.82rem', color: '#5a5a58', margin: 0 }}>{c.desc}</p></div></div>))}</div></div>
        ...
        <section style={{ background: 'var(--g-black)', padding: '3rem 2rem', borderTop: '4px solid var(--g-red)' }}>
          <div style={{ maxWidth: '900px', margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '2rem', flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', gap: '1rem', flexShrink: 0 }}><a href="tel:+33781074746" className="btn-red"><Phone size={16} /> Nous appeler</a><a href="mailto:president@galactusdigital.com" className="btn-ghost"><Mail size={16} /> Demander un devis</a></div>
          </div>
        </section>
        {/* Responsive mobile */}
        <style>{`
