...
export const Route = createFileRoute('/formations/ccna-cisco')({
  component: FormationPage,
  head: () => ({
    meta: [
      { title: "CCNA v2.0 — Cisco (parcours accéléré) 200-301 | GALACTUS Digital" },
      { name: "description", content: "CCNA v2.0 (200-301) — parcours accéléré officiel Cisco, 5 jours. Nouvelle section IA, virage troubleshooting. Distinct du parcours NetAcad. Qualiopi." },
    ],
  }),
})
...
const DATA = {
  editeur: 'Cisco',
  famille: 'Cisco — CCNA (parcours officiel accéléré)',
  titre: 'CCNA v2.0 — Cisco (parcours accéléré)',
  sousTitre: 'Implementing and Administering Cisco Solutions · Examen 200-301 blueprint v2.0 · Distinct du parcours NetAcad (ITN·SRWE·ENSA)',
  ref: 'CCNA-CISCO-V2',
  ...
  objectifs: [
    ...
    { text: "Diagnostiquer et résoudre les incidents réseau (virage troubleshooting de la v2.0)", neuf: true },
    { text: "Comprendre le rôle de l'IA générative et prédictive dans les opérations réseau", neuf: true },
    "Mettre en œuvre la sécurité réseau de base et la vérification",
    "Découvrir la programmabilité, l'automatisation (Ansible, Terraform) et le SDN",
  ] as Item[],
  public: [
    "Professionnels des réseaux et télécoms",
    "Techniciens support IT visant la certification CCNA",
    "Administrateurs systèmes en reconversion réseau",
    "Toute personne débutant une carrière réseau (accessible aux personnes en situation de handicap sachant utiliser un ordinateur)",
  ],
  prerequis: [
    "Connaissances de base en informatique et navigation dans le système d'exploitation",
    "Compétences de base en utilisation d'Internet",
    "Notions de base réseau et du protocole TCP/IP",
    "Contrôle des prérequis par questionnaire + vérification à l'accès plateforme (séance de remise à niveau si besoin)",
  ],
  modules: [
    { num: 'J1', titre: 'Fondamentaux réseau & accès', duree: '8h', contenu: [
      "Composants réseau, topologies physiques/logiques, modèles OSI et TCP/IP",
      "Adressage IPv4, sous-réseaux (VLSM), adressage IPv6",
      "Commutation Ethernet, VLAN et trunking (802.1Q)",
      "TP — configuration de base des commutateurs",
    ] as Item[] },
    { num: 'J2', titre: 'Connectivité IP & routage', duree: '8h', contenu: [
      "Routage statique et par défaut IPv4/IPv6",
      "OSPF simple aire — concepts et configuration",
      "Redondance de premier saut (HSRP)",
      "TP — routage inter-VLAN et OSPF",
    ] as Item[] },
    { num: 'J3', titre: 'Services IP & sécurité', duree: '8h', contenu: [
      "DHCP, DNS, NAT/PAT, NTP, SNMP, Syslog",
      "ACL standard et étendues, sécurité des ports",
      "Sécurité des équipements et bonnes pratiques",
      "TP — services IP et filtrage",
    ] as Item[] },
    { num: 'J4', titre: 'Sans-fil, automatisation & SDN', duree: '8h', contenu: [
      "Réseaux sans fil (WLC), architecture et sécurité WPA2/WPA3",
      { text: "Automatisation et gestion de configuration : Ansible et Terraform", neuf: true },
      "Programmabilité, APIs REST, JSON et principes SDN",
  ...
  examen: {
    ...
    format: 'QCM, glisser-déposer, simulations et dépannage',
    score: 'Défini par Cisco (variable)',
    note: `« CCNA v2.0 » désigne la version du blueprint d'examen 200-301 (à ne pas confondre avec la version du courseware officiel, numérotée séparément). Le blueprint v2.0 entre en vigueur le 3 février 2027 (la v1.1 est proposée jusqu'au 2 février 2027) ; le code d'examen reste 200-301. La certification CCNA est valable 3 ans. Le passage de l'examen (voucher officiel Pearson VUE) est inclus dans le tarif de la formation.`,
  },
  methodes: [
    "Formation dispensée par un formateur Cisco CCNP / CCDP certifié (15+ ans d'expérience, administrateur VUE Pearson)",
    "Format accéléré : 5 jours intensifs, présentiel ou FOAD synchrone",
    "Labs pratiques sur simulateurs et équipements (Packet Tracer / CML)",
    "Supports officiels Cisco et QCM d'entraînement à l'examen",
    "Quiz de validation des acquis à chaque fin de journée",
    ...
    "Préparation intensive à l'examen officiel Cisco 200-301 v2.0",
...
// Badge « NOUVEAU v2.0 » — signale les ajouts du blueprint d'examen CCNA 200-301 v2.0.
function NewBadge() {
  return (<span style={{ display: 'inline-flex', alignItems: 'center', flexShrink: 0, fontFamily: 'var(--font-title)', fontSize: '0.55rem', fontWeight: 700, letterSpacing: '0.10em', textTransform: 'uppercase', color: '#fff', background: 'var(--g-red)', padding: '1px 6px', marginLeft: '0.4rem', borderRadius: '2px', verticalAlign: 'middle' }}>Nouveau v2.0</span>)
}
...
function FormationPage() {
  const FORMATION = getFormationBySlug('ccna-cisco')
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(175px,1fr))', gap: '0.75rem' }} className="fiche-exam-grid">
                  {[
                    ...
                    <div key={row.label} style={{ padding: '0.75rem', background: 'var(--g-offwhite)', border: '1px solid rgba(187,187,187,0.25)' }}>
                      ...
                      <div style={{ fontSize: '0.84rem', color: 'var(--g-black)', fontWeight: 600 }}>{row.val}{typeof row.val === 'string' && row.val.includes('v2.0') && <NewBadge />}</div>
                    </div>
                  ))}
                </div>
