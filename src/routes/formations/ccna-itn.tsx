...
import {
  CheckCircle, Clock, Calendar, Users, MapPin, Phone, Mail,
  Award, BookOpen, Monitor, Wifi, AlertCircle, Network,
} from 'lucide-react'
import { getFormationBySlug, getTarifsModalites } from '../../data/catalogue'

export const Route = createFileRoute('/formations/ccna-itn')({
  component: CcnaItnPage,
  head: () => ({
    meta: [
      { title: "CCNA Module 1/3 — Introduction aux réseaux Cisco | GALACTUS Digital" },
      { name: "description", content: "CCNA Introduction aux réseaux ITN Module 1/3. 70h. Certification CCNA 200-301. OSI TCP/IP IPv4 IPv6 CLI Cisco. Territoires ultramarins. Qualiopi." },
    ],
  }),
})

// ─── Données de la formation ──────────────────────────────────────────────────
// ⚠️  Dupliquer ce fichier pour chaque nouvelle fiche.
//     Modifier uniquement le bloc DATA ci-dessous + le createFileRoute.
...
const DATA = {
  ...
  objectifs: [
    'Décrire le fonctionnement des réseaux et identifier les dispositifs majeurs (switchs, routeurs, pare-feux)',
    'Expliquer le modèle OSI et le modèle TCP/IP, identifier le rôle de chaque couche',
    'Configurer l\'adressage IPv4 et IPv6, calculer des masques de sous-réseau (VLSM)',
    'Configurer des équipements Cisco IOS en ligne de commande (CLI)',
    'Mettre en œuvre les bases de la sécurité réseau et du protocole ARP',
    'Utiliser Cisco Packet Tracer pour concevoir et tester des topologies réseau',
  ],

  // Public
  public: [
    'Administrateurs réseaux débutants',
    'Techniciens support IT',
    'Ingénieurs systèmes en reconversion réseau',
    'Étudiants en BTS SIO / Licence Pro réseaux',
    'Tout professionnel IT souhaitant certifier ses compétences réseau',
  ],

  // Prérequis
  prerequis: [
    'Utilisation courante d\'un PC (Windows ou Linux)',
    'Navigation dans le système de fichiers',
    'Notions basiques de la navigation Internet',
    'Aucune connaissance réseau préalable requise pour ce module 1',
  ],
...
function CcnaItnPage() {
  const FORMATION = getFormationBySlug('ccna-itn')
      ...
      <main style={{ paddingTop: '72px' }}>
          ...
          <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
            ...
            <div style={{ display: 'flex', gap: '2rem', flexWrap: 'wrap' }}>
              {[
                ...
                { icon: <Calendar size={14} />, text: 'Voir calendrier', href: '/calendrier?f=ccna-itn' },
              ].map((m, i) => (
                m.href ? (
        ...
        <section style={{ background: 'var(--g-offwhite)', padding: '4rem 2rem' }}>
          <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
            ...
            <p style={{ fontSize: '0.95rem', color: '#5a5a58', lineHeight: 1.8, maxWidth: '800px' }}>
              {DATA.description}
            </p>
          </div>
        </section>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ marginTop: '1rem', padding: '0.75rem 1rem', background: 'rgba(228,31,38,0.06)', border: '1px solid rgba(228,31,38,0.15)' }}>
                  <p style={{ fontSize: '0.84rem', color: '#5a5a58', margin: 0, lineHeight: 1.6 }}>
                    <strong style={{ color: 'var(--g-black)' }}>Note importante — </strong>{DATA.examen.note}
                  </p>
                </div>
              </div>
            </div>
